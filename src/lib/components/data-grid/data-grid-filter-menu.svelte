...
<Popover bind:open>
  ...
  <PopoverContent
    {align}
    class="flex w-full max-w-(--radix-popover-content-available-width) flex-col gap-3.5 p-4 sm:min-w-120"
  >
    <div class="flex flex-col gap-1">
      ...
      <p
        class={cn("text-sm text-muted-foreground", columnFilters.length > 0 && "sr-only")}
      >
        {columnFilters.length > 0
                    ...
                    <CommandGroup>
                      {#each columns as col (col.value)}
                        <CommandItem
                          ...
                          onSelect={() => {
                            const newVariant =
                              columnVariants.get(col.value) ?? "short-text"
