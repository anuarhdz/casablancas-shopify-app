  ...
  async function removeFile(fileId: string) {
    ...
    if (table.options.meta?.onFilesDelete && rowData) {
      try {
        ...
        toast.error(
          err instanceof Error ? err.message : `Failed to delete ${fileToRemove.name}`
        )
        return
      }
    }
            ...
            <div class="flex flex-col gap-2">
              ...
              <div class="max-h-50 space-y-1 overflow-y-auto">
                {#each files as file (file.id)}
                  ...
                  <div
                    class="flex items-center gap-2 rounded-md border bg-muted/50 px-2 py-1.5"
                  >
  ...
  {:else if files.length > 0}
    <div class="flex flex-wrap items-center gap-1 overflow-hidden">
      {#each visibleFiles as file (file.id)}
        ...
        {:else}
          ...
          <Badge variant="secondary" class="h-5 shrink-0 gap-1 px-1.5 text-xs">
            ...
            <span class="max-w-25 truncate">{file.name}</span>
          </Badge>
        {/if}
      {/each}
      {#if hiddenFileCount > 0}
        <Badge
          variant="outline"
          class="h-5 shrink-0 px-1.5 text-xs text-muted-foreground"
        >
          +{hiddenFileCount}
        </Badge>
      {/if}
